...
def top_k_top_p_min_p_filtering(
    logits: torch.Tensor, top_k: int, top_p: float, min_p: float, min_tokens_to_keep: int = 1
) -> torch.Tensor:
...
def next_buffer(buffer: torch.Tensor, block_size: int, fill=None):
    if fill:
        new_buffer = torch.full(
            (buffer.shape[0], buffer.shape[1] + block_size), fill, dtype=buffer.dtype, device=buffer.device
        )
    else:
        new_buffer = torch.zeros(
            (buffer.shape[0], buffer.shape[1] + block_size), dtype=buffer.dtype, device=buffer.device
        )
...
def generate(
    ...
    # Compile models
    prefill_model = torch.compile(PrefillModel(model), mode="reduce-overhead", fullgraph=True)

    decode_model = torch.compile(DecodeOneTokenModel(model), mode="reduce-overhead", fullgraph=True)
