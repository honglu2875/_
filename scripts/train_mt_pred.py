...
def main(job_config: JobConfig):
    ...
    model = LlamaMTPred(model, stride=STRIDE, num_tokens=NUM_TOKENS)
    ...
    model_config = hf_to_titan_config(model.config)
    ...
    # log model size
    emb_param_count = model_config.dim * model_config.vocab_size
    model_param_count = sum(param.numel() for param in model.parameters()) - emb_param_count
    ...
    logger.info(
        ...
        f"{color.red}size: {model_param_count + emb_param_count:,} total parameters{color.reset}"
    )
    ...
    parallelize_model(model, model_layers, world_mesh, parallel_dims, job_config)
    """
        ...
        while train_state.step < job_config.training.steps:
            ...
            if parallel_dims.pp_enabled:
                ...
                with train_context(optional_context_parallel_ctx):
                    # fwd pass output is `CausalLMOutput`. Need to access `logits` member
                    ce_loss, fut_ce_loss = loss_fn(model, input_ids, labels)
                    # pred.shape=(bs, seq_len, vocab_size)
                    # need to free to before bwd to avoid peaking memory
                    final_loss = ce_loss + FUT_COEFF * fut_ce_loss
                    final_loss.backward()
            ...
            if train_state.step == 1 or train_state.step % job_config.metrics.log_freq == 0:
                if parallel_dims.dp_replicate_enabled or parallel_dims.dp_shard_enabled or parallel_dims.cp_enabled:
                    ...
                    global_avg_loss, global_max_loss = (
                        ...
                        utils.dist_max(final_loss, world_mesh["dp_cp"]),
                    )
                    global_avg_fut_loss = utils.dist_mean(fut_ce_loss, world_mesh["dp_cp"])
                else:
                    global_avg_loss = global_max_loss = final_loss.item()
                    global_avg_fut_loss = fut_ce_loss.item()

                # update train state
                train_state.log_steps.append(train_state.step)
                train_state.global_avg_losses.append(global_avg_loss)
                train_state.global_max_losses.append(global_max_loss)
                train_state.global_avg_fut_losses.append(global_avg_fut_loss)
                ...
                metrics = {
                    ...
                    "loss_metrics/global_avg_fut_loss": global_avg_fut_loss,
